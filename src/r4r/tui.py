...
from textual.widgets import (
    Header, Footer, Log, Static, Button, Input, Select, 
    ProgressBar, Tabs, TabPane, DataTable, ScrollView,
    Switch, Label, Collapsible, Pretty
)
...
class LogViewerApp(App):
    ...
    def _setup_streams_table(self) -> None:
        ...
        detail_table.add_columns("ID", "Name", "Service ID", "Filters", "Enabled", "Created")
    ...
    def _display_log_entry(self, log_entry: LogEntry, log_widget: Log) -> None:
        ...
        # Source info
        source = log_entry.source[:10] + "..." if len(log_entry.source) > 10 else log_entry.source
    ...
    def _should_filter_log(self, log_entry: LogEntry) -> bool:
        ...
        # Level filter
        if self.current_filters.get("level") and log_entry.level != self.current_filters["level"]:
            return True
    ...
    def _update_status(self) -> None:
        ...
        self.query_one("#filtered-count", Static).update(f"🔍 Filtered: {self.filtered_count}")
